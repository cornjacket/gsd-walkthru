...
| 3 | missing-header guard (Stripe) | `src/providers/stripe.ts`: `if (header === undefined)` block commented out | integration (gap closed post-experiment) | **As originally run (unit-tier):** the missing-header integration test only asserted HTTP 401 status, and without the `undefined` guard the request falls through to the `typeof header !== 'string'` guard which also throws 401 (`invalid_signature_format` reason instead of `missing_header`). The status remained 401 so the integration test passed; only the unit test (`src/providers/stripe.test.ts > stripeProvider.validate() > missing Stripe-Signature header throws missing_header`) caught the reason shift. **Gap closed (WR-01):** the integration test now asserts the reason — `tests/integration/stripe.test.ts:80` `expect(res.body.reason).toBe('missing_header')`. Re-running this mutation now fails the integration test `tests/integration/stripe.test.ts > Stripe integration — rawBodyCapture mode (BODY-01) > missing Stripe-Signature header: returns 401 with reason missing_header`. | yes |
...
Mutation 3 surfaces a coverage gap in the integration suite: the `missing Stripe-Signature header` integration test only checks the HTTP status code (401), not the specific `WebhookValidationError.reason`. Because both `'missing_header'` and `'invalid_signature_format'` map to HTTP 401, removing the `header === undefined` guard silently shifts the error reason without changing the observable HTTP status. The integration test passes while the security contract changes (the reason code diverges from the correct `'missing_header'`). The unit test catches this because it asserts the exact `reason` field. This gap was noted but not corrected during the experiment itself (no source changes per D-19 discipline).

**Closure (post-experiment, WR-01):** the gap was subsequently closed by adding a reason assertion to the missing-header integration test — `tests/integration/stripe.test.ts:80` now asserts `res.body.reason === 'missing_header'`. Re-running Mutation 3 against the current tree fails this integration test, so the missing-header guard is now demonstrated at the integration tier (row 3 Tier updated accordingly). All 5 guard mutations are now caught by integration tests.
