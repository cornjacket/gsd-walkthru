...
- [x] Comprehensive test coverage (unit + integration against a real Express app) — Validated in Phase 6 (QUAL-01..04): Supertest integration suite for all three providers under both body-parser modes; an **enforcing** per-file >90% V8 coverage gate (`src/crypto/`, `src/providers/`, `src/middleware.ts`) wired into CI on the Node 22 × Express 5.x cell; and a negative-case audit closing the 8 Phase 4/5 carry-over advisories, proven non-vacuous by a 5-guard mutation experiment. 139 tests across 16 files. Note: the coverage gate was initially misconfigured (block outside `test:`, silently non-enforcing) — caught by independent verification and fixed in-phase.
- [x] Polished README with installation, per-provider quickstarts, and security notes — Validated in Phase 7 (DOCS-01/DOCS-02): README restructured into canonical OSS onboarding shape (installation → three copy-pasteable per-provider quickstarts → full configuration reference table for all four exports → security notes with an honest per-provider replay-protection table). Stale "Phase 3 of 7 / validators stubbed" banner replaced with v1.0/Ready-to-use framing; package.json + `src/index.ts` VERSION bumped to 1.0.0 for self-consistency.
- [x] Runnable example Express app demonstrating all three providers end-to-end — Validated in Phase 7 (DOCS-03): `examples/example-app/` boots via `npm start`, fires three independently-signed mock webhooks (Node `crypto.createHmac`, not the library's helper, per D-09), prints each validated `req.webhook` to stdout, and exits 0.
...
---
*Last updated: 2026-06-04 after Phase 7 (Documentation & Example App) completion — **v1.0 milestone complete (7/7 phases)**. README restructured into shippable OSS onboarding doc (installation, three per-provider quickstarts, full config reference, honest per-provider replay-protection security table); package.json + `src/index.ts` VERSION bumped to 1.0.0; runnable `examples/example-app/` boots and validates all three providers end-to-end (exit 0) with independent Node-crypto mock senders (D-09). DOCS-01/02/03 validated; library 139 tests across 16 files still green. Phase verified PASSED 10/10. Prior context: 2026-05-29 after Phase 6 (Integration Tests, Coverage Gate & Negative-Case Audit) completion — Supertest integration suite across all three providers under both body-parser modes, an enforcing per-file >90% V8 coverage gate in CI (Node 22 × Express 5.x cell), and a negative-case audit (8 Phase 4/5 carry-over advisories closed) proven non-vacuous by a 5-guard mutation experiment. QUAL-01..04 validated; 139 tests across 16 files. Independent verification caught a misconfigured (non-enforcing) coverage gate and a stripe type-soundness regression, both fixed in-phase. Phase 7 (README + runnable example app) remains. Prior context: 2026-05-08 after Phase 5 (GitHub & Shopify Providers) completion — real `githubProvider.validate()` (HMAC-SHA256 over rawBody Buffer-direct, `sha256=<hex>` parse, deprecated SHA-1 header invisible per D-02, `eventId === deliveryId`) and `shopifyProvider.validate()` (HMAC-SHA256 over rawBody Buffer-direct, loose base64 decode, hex-as-base64 rejected via Phase 2 D-09 length-mismatch → `'signature_mismatch'`, `eventId === webhookId`). Phase 5 shipped Phase 4 WR-03 fix (D-10 three-way auth-header split — array → `'invalid_signature_format'`, not folded into `'missing_header'`) and Phase 4 WR-02 fix (D-14 mandatory outer-`expect(toThrow)` guard) from day one in BOTH new providers. NO new public exports, NO `WebhookValidationReason` union widening, NO touches to errors/middleware/index/registry/types/stripe — only `src/providers/{github,shopify}.{ts,test.ts}`. 110 tests green across 13 files (+24 from 86/11 baseline: 13 GitHub + 11 Shopify). 3 advisory code-review warnings (empty-string asymmetry untested, leakage assertion shape, leakage assertion not on tampered path) tracked in `05-REVIEW.md` for Phase 6 negative-case audit. Phase 6 (integration tests + coverage gate + cross-provider audit including the 5 advisory carry-overs from Phase 4 + 3 from Phase 5) and Phase 7 (README + example app) remain.*
